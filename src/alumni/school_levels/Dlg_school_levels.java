 ...
 * @author Guinness
 */
public class Dlg_school_levels extends javax.swing.JDialog {
...
//</editor-fold>
    ...
    private Dlg_school_levels(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        setUndecorated(true);
        initComponents();
        myInit();
    ...
    private Dlg_school_levels(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        setUndecorated(true);
        initComponents();
        myInit();
    ...
    private Dlg_school_levels myRef;

    private void setThisRef(Dlg_school_levels myRef) {
        this.myRef = myRef;
    }
    private static java.util.Map<Object, Dlg_school_levels> dialogContainer = new java.util.HashMap();
    ...
    public static Dlg_school_levels create(java.awt.Window parent, boolean modal) {
    ...
    public static Dlg_school_levels create(java.awt.Window parent, java.awt.Dialog.ModalityType modalType) {

        if (parent instanceof java.awt.Frame) {

            Dlg_school_levels dialog = dialogContainer.get(parent);

            if (dialog == null) {
                dialog = new Dlg_school_levels((java.awt.Frame) parent, false);
        ...
        if (parent instanceof java.awt.Dialog) {
            ...
            if (dialog == null) {
                dialog = new Dlg_school_levels((java.awt.Dialog) parent, false);
                dialog.setModalityType(modalType);
                dialogContainer.put(parent, dialog);
                java.util.logging.Logger.getAnonymousLogger().log(Level.INFO, "instances: {0}", dialogContainer.size());
                dialog.setThisRef(dialog);
    ...
    public static void main(String args[]) {
        ...
        Dlg_school_levels dialog = Dlg_school_levels.create(new javax.swing.JFrame(), true);
...
//                btn_0.doClick();
    ...
    public static void loadData_official_schedule_types(List<School_levels.to_school_levels> acc) {
    ...
    public static class Tblofficial_schedule_typesModel extends AbstractTableAdapter {
        ...
        public static String[] COLUMNS = {
            "Level", "", "", "created_by", "updated_by", "status", "schedule_type"
        };
        ...
        public Object getValueAt(int row, int col) {
            School_levels.to_school_levels tt = (School_levels.to_school_levels) getRow(row);
            switch (col) {
                case 0:
                    return " " + tt.level;
                case 1:
    ...
    private void ret_official_schedule_types() {
        String where = "";
        List<School_levels.to_school_levels> datas = School_levels.ret_data(where);
    ...
    private void save_official_schedule_types() {
        ...
        if (row < 0) {
            ...
            int status = 0;
            int upload_status = 0;
            String level = jTextField1.getText();
            School_levels.to_school_levels level1 = new School_levels.to_school_levels(id, created_at, updated_at, created_by, updated_by, status, upload_status, level);
            School_levels.add_data(level1);
            ret_official_schedule_types();
            Alert.set(1, level);
            clear_official_schedule_types();
        } else {
            School_levels.to_school_levels to = (School_levels.to_school_levels) tbl_official_schedule_types_ALM.get(row);
            int id = to.id;
            ...
            String level = jTextField1.getText();
            School_levels.to_school_levels level1 = new School_levels.to_school_levels(id, created_at, updated_at, created_by, updated_by, status, upload_status, level);
            School_levels.update_data(level1);
            ret_official_schedule_types();
            Alert.set(2, level);
            clear_official_schedule_types();
        }
    ...
    private void select_official_schedule_types() {
        ...
        final School_levels.to_school_levels to = (School_levels.to_school_levels) tbl_official_schedule_types_ALM.get(row);
        if (col == 1) {

            jTextField1.setText(to.level);
        }
        if (col == 2) {
